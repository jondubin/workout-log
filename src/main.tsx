...
import { loadLogs, saveLogs, type Pattern, type SetLog } from "./log-store";
import "./style.css";
...
function App() {
  ...
  useEffect(() => {
    const initialize = async () => {
      try {
        ...
        setLogs(await loadLogs()); setStatus("");
  ...
  const persist = async (next: SetLog[]) => { setLogs(next); setStatus("Saving…"); try { await saveLogs(next); setStatus(""); } catch { setStatus("Save failed. Check Dropbox and try again."); } };
  const addSets = () => { const valid = reps.filter((rep) => rep.trim()); if (!valid.length) return; void persist([...logs, ...valid.map((rep) => ({ id: crypto.randomUUID(), date, exercise: selected.name, pattern: selected.pattern, reps: rep.trim(), notes }))]); setReps([""]); setNotes(""); };
  ...
  return <main>
    ...
    <section className="history"><h2>Exercise history</h2>{history.map(([day, entries]) => <div className={`history-day${day === date ? " current-day" : ""}`} key={day}><strong>{day === date ? `Today · ${formatDate(day)}` : formatDate(day)}</strong>{entries.length === 0 ? <p className="empty">No sets logged yet.</p> : <ul>{entries.map((entry) => <li key={entry.id}>{entry.exercise} · {entry.reps} reps{entry.notes ? ` · ${entry.notes}` : ""}<button onClick={() => void persist(logs.filter((item) => item.id !== entry.id))}>Delete</button></li>)}</ul>}</div>)}</section>
