...
function App() {
  ...
  const addSets = () => { const valid = reps.filter((rep) => rep.trim()); if (!valid.length) return; void persist([...logs, ...valid.map((rep) => ({ id: crypto.randomUUID(), date, exercise: selected.name, pattern: selected.pattern, reps: rep.trim() }))]); setReps([""]); };
  const updateDayNote = (note: string) => { const next = { ...dayNotes, [date]: note }; void persist(logs, next); };
  const changeCount = (amount: number) => setReps((current) => amount > 0 ? [...current, ""] : current.length > 1 ? current.slice(0, -1) : current);
  ...
  return <main>
    ...
    <section className="add"><h2>Add sets for today</h2><div className="form"><label>Exercise<select value={exercise} onChange={(event) => setExercise(event.target.value)}>{(["push", "pull", "legs"] as Pattern[]).map((pattern) => <optgroup key={pattern} label={patternNames[pattern]}>{exercises.filter((item) => item.pattern === pattern).map((item) => <option key={item.name}>{item.name}</option>)}</optgroup>)}</select></label><div className="set-count"><span>Sets</span><button onClick={() => changeCount(-1)}>−</button><b>{reps.length}</b><button onClick={() => changeCount(1)}>+</button></div><label className="reps">Reps<input inputMode="numeric" value={reps[0]} placeholder="Set 1" onChange={(event) => setReps((current) => current.map((value, index) => index === 0 ? event.target.value : value))} /></label><button className="add-button" onClick={addSets}>Add to today</button>{reps.length > 1 && <div className="extra-reps">{reps.slice(1).map((rep, index) => <input key={index + 1} inputMode="numeric" value={rep} placeholder={`Set ${index + 2}`} onChange={(event) => setReps((current) => current.map((value, currentIndex) => currentIndex === index + 1 ? event.target.value : value))} />)}</div>}</div><p className="hint">Each row is one hard set. Add another batch later if you train again.</p><label className="day-notes">Notes for this day<textarea value={dayNotes[date] ?? ""} placeholder="How did it feel? Anything to remember?" onChange={(event) => updateDayNote(event.target.value)} /></label></section>
