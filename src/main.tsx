...
function App() {
  ...
  return <main>
    <header><h1>Exercise log</h1><div className="header-right"><div className="date-controls"><button aria-label="Previous day" onClick={() => setDate((value) => shiftDate(value, -1))}>←</button><label>Date <input type="date" value={date} max={localDate()} onChange={(event) => setDate(event.target.value)} /></label>{!isCurrentDay && <button aria-label="Next day" onClick={() => setDate((value) => shiftDate(value, 1))}>→</button>}</div><button className="theme-toggle" onClick={() => setDark((value) => !value)}>{dark ? "Light" : "Dark"}</button><button className="link-button" onClick={() => { logout(); window.location.reload(); }}>Disconnect</button></div></header>
    ...
    <section className="history"><h2>Exercise history</h2>{history.map(([day, entries]) => <div className={`history-day${day === date ? " current-day" : ""}`} key={day}><strong>{day === date ? `Today · ${formatDate(day)}` : formatDate(day)}</strong><div>{entries.length === 0 ? <p className="empty">No sets logged yet.</p> : <ul>{entries.map((entry) => <li key={entry.id}>{entry.exercise} · {entry.reps} reps<button onClick={() => void persist(logs.filter((item) => item.id !== entry.id))}>Delete</button></li>)}</ul>}{day !== date && dayNotes[day] && <p className="saved-day-note">{dayNotes[day]}</p>}</div></div>)}</section>
